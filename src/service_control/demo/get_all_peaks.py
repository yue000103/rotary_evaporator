...
async def start_experiment(task_ctrl: TaskController, params_1: dict,big_bottle_volume,small_bottle_volume,column_id,wash_time_min,
...
async def small_to_xuanzhegn(task_ctrl: TaskController, params_1: dict,big_bottle_volume,small_bottle_volume,column_id,wash_time_min,
                  experiment_time_min,sample_id,penlin_time_s,peak_number,small_position_id,big_position_id,warehouse_id,
                  sample_volume, xuanzheng_timeout_min):
...
async def big_to_xuanzheng(task_ctrl: TaskController, params_1: dict,big_bottle_volume,small_bottle_volume,column_id,wash_time_min,
                  experiment_time_min,sample_id,penlin_time_s,peak_number,small_position_id,big_position_id,warehouse_id,
                  sample_volume, xuanzheng_timeout_min):
...
async def clean_and_transfer(task_ctrl: TaskController, params_1: dict,big_bottle_volume,small_bottle_volume,column_id,wash_time_min,
                  experiment_time_min,sample_id,penlin_time_s,peak_number,small_position_id,big_position_id,warehouse_id,
                  sample_volume, xuanzheng_timeout_min):
...
async def collect(task_ctrl: TaskController, params_1: dict,big_bottle_volume,small_bottle_volume,column_id,wash_time_min,
                  experiment_time_min,sample_id,penlin_time_s,peak_number,small_position_id,big_position_id,warehouse_id,
                  sample_volume, xuanzheng_timeout_min):
    try:
        ...
        code = sepu_api.select_retain_tubes_by_id(peak_number)
        if code == 600:
            print("无峰出现，清空试管")
...
async def main():
    ...
    await start_experiment(task_ctrl=task_ctrl, params_1=params["params"],big_bottle_volume=params["big_bottle_volume"],small_bottle_volume=params["small_bottle_volume"]
    ...
    for i in range(peaks_num):
        ...
        if i == 0:
            code = await collect(**args)
            if code == 600:
                break
            await big_to_xuanzheng(**args)
            await clean_and_transfer(**args)
            if i == peaks_num - 1:
                await small_to_xuanzhegn(**args)
                break
            ...
            await big_to_xuanzheng(**args)
            await clean_and_transfer(**args)
            small_to_xuanzhegn_task = asyncio.create_task(small_to_xuanzhegn(**args))
            ...
            if code == 600:
                break
            await asyncio.gather(small_to_xuanzhegn_task)
            await big_to_xuanzheng(**args)
            await clean_and_transfer(**args)
            robot_controller.clean_to_collect()
