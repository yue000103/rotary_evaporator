...
class SepuService:
    ...
    def is_terminated(self):
        ...
        判断实验是否终止
        """
        status = self.sepu_api.get_experiment_status()
        return status.get("terminate_flag", 0) == 1
    ...
    def is_paused(self):
        ...
        """
        status = self.sepu_api.get_experiment_status()
        return status.get("pause_flag", 0) == 1

    def waiting_exeperiment_terminating(self):
        """
        等待实验终止
        """
